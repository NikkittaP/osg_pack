...
set MS_BUILD_EXE="C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\MSBuild\Current\Bin\MSBuild.exe"
...
set BUILD_3RDPARTY_DIR=%BASE_3RDPARTY_DIR%\build_2019
set LOG_BUILD_3RDPARTY_DIR=%BUILD_3RDPARTY_DIR%\_build_log
set INSTALL_3RDPARTY_DIR=%BASE_3RDPARTY_DIR%\install_2019
...
call :StartTimer
    ...
    echo --------------------------------------------------
    echo.
    echo Building on %NUM_THREADS% threads with Visual Studio 2019:
...
:: GDAL
    ...
    if not %IS_GDAL_BUILD% EQU 1 (
        if %IS_BUILD_RELEASE% EQU 1 (
            cd gdal
            xcopy /Y /Q nmake_release_2019.opt nmake.opt > nul
        ...
        if %IS_BUILD_DEBUG% EQU 1 (
            ...
            cd gdal_debug
            xcopy /Y /Q nmake_debug_2019.opt nmake.opt > nul
