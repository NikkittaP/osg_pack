...
** [21] jx  12Feb04 nCounter is added to QmfRegion to fix rounding error in erw_decompress_read_region_line_bil().
...
**	UINT8	blocking format (NCS_BLOCKING_LEVEL = blocking individual levels)
...
//#define	NCSFree(ptr)		if( ptr ) NCSFree(ptr);
...
// 16bit version
#define NUM_SYMBOL_VALUES16 _NUM_SYMBOL_VALUES16
#define SIGN_MASK16	0x40000000
#define RUN_MASK16	0x80000000
#define MAX_BIN_VALUE16	1073741823	// 0x3fffffff
#define MIN_BIN_VALUE16	-1073741823	// Prior to masking
#define VALUE_MASK16	0x3fffffff		/* without the sign or run bits set */
#define MAX_RUN_LENGTH16 0x7fffffff	/* 2147483647, can't be longer than this or would run into mask bits */
...
#ifdef NOTDEF
typedef enum {
	NCS_ENCODE_INVALID	= 0,	// invalid encoding style
	NCS_ENCODE_RAW		= 1,	// data is not encoded, and is in a raw format
	NCS_ENCODE_HUFFMAN	= 2,	// zero's run length encoded, and then huffman encoded
	NCS_ENCODE_RANGE	= 3,	// range encoding
	NCS_ENCODE_RANGE8	= 4,	// 8 bit difference then range encoding
	NCS_ENCODE_ZEROS	= 5,	// encode the entire block as zero's
	NCS_ENCODE_RUN_ZERO	= 6,	// Run length encoded as zero's
	NCS_ENCODE_RANGE16	= 7		// 16 bit difference then range encoding
} NCSEncodeFormat;
#endif
typedef UINT16 NCSEncodeFormat;
#define	NCS_ENCODE_INVALID	0
#define	NCS_ENCODE_RAW		1
#define	NCS_ENCODE_HUFFMAN	2
#define	NCS_ENCODE_RANGE	3
#define	NCS_ENCODE_RANGE8	4
#define	NCS_ENCODE_ZEROS	5
#define	NCS_ENCODE_RUN_ZERO	6
#define	NCS_ENCODE_RANGE16	7	// 16 bit difference then range encoding
#define	NCS_ENCODE_HUFFMAN16	8	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 2
#define	NCS_ENCODE_HUFFMAN8to8	9	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 3 or pack_data8to8() when nECWhuffmancodingFlag != 0
#define	NCS_ENCODE_RUN_ZERO_COUNT_LOC_SHIFT				10	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 4 and 5
#define	NCS_ENCODE_HUFFMAN8to8_COUNT_LOC_SHIFT			11	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 4
#define	NCS_ENCODE_HUFFMAN16to8_COUNT_LOC_SHIFT			12	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 5
#define	NCS_ENCODE_RUN_NONDIFF_COUNT_LOC_SHIFT			13	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 6 and 7
#define	NCS_ENCODE_HUFFMAN8to8_NONDIFF_COUNT_LOC_SHIFT	14	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 6
#define	NCS_ENCODE_HUFFMAN16to8_NONDIFF_COUNT_LOC_SHIFT	15	// 16 bit for a symbol. Used for pack_data16() when nECWhuffmancodingFlag = 7
...
#define NCSECW_BUFFER_TYPE INT16
#define NCSECW_BUFFER_TYPE_32 INT32 // for 16bit
#define NCSECW_BUFFER_FRAC_BITS 3
...
**	NOTE!! NCSECWClient.h has this structure, defined as "NCSFileInfo".
...
/*
**
**	The QmfLevelBand is only allocated for COMPRESSION. Because the bin size is
...
/*
**
**	The QmfLevelBand16 is only allocated for COMPRESSION. Because the bin size is
**	needed for decompression, it is pulled out of the band structure (binsize can be
**	different for each band) and put into the main level structure. So the bin_size
**	is the only multi-band value in the QMF level structure; the rest of the band
**	specific information is held here at the band level. This also makes the QMF
...
**	including the top level, is easier to structure.
**
**  This is 16bit version of QmfLevelBand
*/

typedef struct QmfLevelBand16 {
	//
	// These buffers are not allocated for the largest (file level) QMF level
	//
	IEEE4	*p_p_lo_lines[FILTER_SIZE+1];	// pointer to enough larger input lines for lowpass input
	...
	UINT8 ***p_p_p_hh_segs;
		/* These are used when p_file_qmf->bLowMemCompress is TRUE (1 line) or FALSE (Y_BLOCK_SIZE lines) [20]*/
	INT32	*p_quantized_output_ll_block;	// Only allocated for level 0 LL, NULL for all other levels
	INT32	*p_quantized_output_lh_block;	// LH block
	INT32	*p_quantized_output_hl_block;	// HL block
	INT32	*p_quantized_output_hh_block;	// HH block
...
typedef struct QmfLevel {
	...
	QmfLevel	*p_larger_qmf; 
	QmfLevel	*p_smaller_qmf;
	QmfLevel	*p_top_qmf;		/* pointer to top (smallest) level QMF */
	QmfLevel	*p_file_qmf;	/* pointer to the fake (largest) file level QMF */
	...
	NCSBlockingFormat blocking_format;		// typically NCS_BLOCKING_LEVEL
	NCSCompressFormat compress_format;	// typically COMPRESS_UINT8 or COMPRESS_YIQ
...
//	NCS_FILE_HANDLE	outfile;					/* output file */
	NCS::CIOStream *pOutFile;
	NCS::CIOStream *pOpacityFile;
	BOOLEAN is_opacity;				/* is an opacity stream */
	BOOLEAN write_non_opacity;		/* for writing non opacity channels in ECW */
	BOOLEAN write_opacity;		/* for writing opacity channels in ECW */
	...
	QmfLevelBand	*p_bands;

	// These buffers are not allocated for the largest (file level) QMF level
	// These are also ONLY allocated for compression, not for decompression. There is one
	// per band being compressed
	// This is 16bit version of p_bands
	QmfLevelBand16	*p_bands16;
	// Optional ECW huffman coding flag for 16 bits. more details see pack_data16() at ECW.h
	UINT8			nECWhuffmancodingFlag;
	...
	NCS::CString 	*tmp_fname;					// temporary file to write this level's blocks, so we can set up block order
	NCS::CIOStream *pTmpFile;
	NCS::CString 	*tmp_fname_opacity;			// temporary file to write this level's blocks, so we can set up block order
	NCS::CIOStream *pTmpFileOpacity;

	// Tile based compression // this could go in CResolution
	NCS::SDK::CIndexedDataFile *pTempLLData;
	UINT64 *pCompletedBlocks;
	...
	UINT64		file_offset_opacity;			// offset to start of this level in the file
	UINT64		first_stream_length;	// offset to the beginning of the opacity stream 
	...
	NCSFileInfo	*pFileInfo;			// Valid at top level only

	//This is used for region update
	UINT32 update_block_x_start;
	UINT32 update_block_y_start;
	UINT32 update_block_x_end;
	UINT32 update_block_y_end;
...
#define DECOMP_LEVEL_32_LINE01(p_level, band, sideband, line)	\
		((p_level)->buffer_32_ptr + (((2 * (band * MAX_SIDEBAND + sideband)) + (line))	* ((p_level)->level_size_x+2)))
// Used to compute index for a given band and subband
#define	DECOMP_INDEX	\
		(band * MAX_SIDEBAND + sideband)

typedef struct QmfRegionLevel {
	UINT32	used_bands;
	NCSECW_BUFFER_TYPE	**p_p_line0;			// [05] this level's LL, LH, HL and HH for line N+0. Must be signed
	NCSECW_BUFFER_TYPE	**p_p_line1;			// [05] this level's LL, LH, HL and HH for line N+1. Must be signed
	NCSECW_BUFFER_TYPE	**p_p_line1_ll_sideband;// [05] This level's LL sideband with x_reflection for line 1, all bands

	// for 16 bit version, but use INT32
	NCSECW_BUFFER_TYPE_32	**p_p_32_line0;			// [05] this level's LL, LH, HL and HH for line N+0. Must be signed
	NCSECW_BUFFER_TYPE_32	**p_p_32_line1;			// [05] this level's LL, LH, HL and HH for line N+1. Must be signed
	NCSECW_BUFFER_TYPE_32	**p_p_32_line1_ll_sideband;// [05] This level's LL sideband with x_reflection for line 1, all bands
	...
	QmfLevel	*p_qmf;				// handy pointer to the QMF for this level
	struct QmfRegion *p_region;	// handy pointer to the region that contains these levels
	...
	NCSECW_BUFFER_TYPE	*buffer_ptr;			// [05] Band/sideband/line[01] buffer.
	// for 16bit version
	NCSECW_BUFFER_TYPE_32	*buffer_32_ptr;			// [05] Band/sideband/line[01] buffer.
...
namespace NCS {
	class CView;
	class CView_CWriterBase;
	namespace ECW {
		class CFile;
	}
}
// One of these is created for each start_region()/end_region() call pair
typedef struct QmfRegion {
	QmfLevel	*p_top_qmf;		// pointer to the smallest level in the QMF
	QmfLevel	*p_largest_qmf;	// pointer to the largest level we have to read for this region resolution
	...
	QmfRegionLevel	*p_levels;
	NCSECW_BUFFER_TYPE	**p_p_ll_line;			// [05] the power of two size, INT32 format, output line. Must be signed
	NCSECW_BUFFER_TYPE	*p_ll_buffer;			// [05] the buffer that the above indexes into
	// for 16bit version
	NCSECW_BUFFER_TYPE_32	**p_p_32_ll_line;			// [05] the power of two size, INT32 format, output line. Must be signed
	NCSECW_BUFFER_TYPE_32	*p_32_ll_buffer;			// [05] the buffer that the above indexes into
	...
	NCS::CView *pNCSFileView;	// [02] pointer to the NCS File View for this QMF Region
	// bAddTextureNoise has been moved to file.h
...
extern NCSError setup_qmf_tree(QmfLevel **pp_qmf, struct EcwCompressionTask *pCompressionTask,
						  UINT32 nBlockSizeX, UINT32 nBlockSizeY, 
						  UINT16 *num_levels, UINT32 x_size, UINT32 y_size, UINT32 nr_bands,
						  UINT32 nInputBands, NCSCompressHint eCompressHint,
						  IEEE4 compression_factor, NCSCompressFormat compress_format, int compress, bool write_non_opacity, bool write_opacity,
						  UINT8 nWriteCompressVersion, bool allocate_buffers);
// for image with 16 bit bands except opacity band
extern NCSError setup_qmf_tree16(QmfLevel **pp_qmf, struct EcwCompressionTask *pCompressionTask,
						  UINT32 nBlockSizeX, UINT32 nBlockSizeY, 
						  UINT16 *num_levels, UINT32 x_size, UINT32 y_size, UINT32 nr_bands,
						  UINT32 nInputBands, NCSCompressHint eCompressHint,
						  std::vector<IEEE4> compression_factor_list, NCSCompressFormat compress_format, int compress, bool write_non_opacity, bool write_opacity,
						  UINT8 nWriteCompressVersion, UINT8 nECWhuffmancodingFlag, bool allocate_buffers);
#endif

extern void delete_qmf_levels(QmfLevel *p_qmf);
extern UINT32 get_qmf_tree_nr_blocks( QmfLevel *p_top_level );

QmfLevel *new_qmf_level(UINT32 nBlockSizeX, UINT32 nBlockSizeY,
						UINT16 level, UINT32 x_size, UINT32 y_size, UINT32 nr_bands,
						QmfLevel *p_smaller_qmf, QmfLevel *p_larger_qmf, int compress);

NCSError allocate_qmf_buffers(QmfLevel *p_top_qmf, int compress);
// 16bit version
NCSError allocate_qmf_buffers16(QmfLevel *p_top_qmf, int compress);



// Data unpack routines

extern int unpack_ecw_block(QmfLevel *pQmfLevel, UINT32 nBlockX, UINT32 nBlockY,
					 UINT8 **ppUnpackedECWBlock, UINT32	*pUnpackedLength,
					 UINT8 *pPackedECWBlock, UINT32 nPackedLength);
int align_ecw_block(NCS::ECW::CFile *pFile, NCSBlockId nBlockID, 
					 UINT8 **ppAlignedECWBlock, UINT32	*pAlignedLength,
					 UINT8 *pPackedBlock, UINT32 nPackedLength);

extern int	unpack_data(UINT8 **p_unpacked,
						UINT8  *p_packed, 
...
int unpack_init_lines( QmfRegionLevel *p_level );

int	unpack_line( QmfRegionLevel *p_level );

void unpack_finish_lines( QmfRegionLevel *p_level );

void unpack_free_lines( QmfRegionLevel *p_level );

// 16bit version
extern int	unpack_data16(UINT8 **p_unpacked,
						UINT8  *p_packed, 
						UINT32 unpacked_length,
						UINT32 packed_length,
						UINT8 nSizeOfEncodeFormat);

int unpack_init_lines16( QmfRegionLevel *p_level );

int	unpack_line16( QmfRegionLevel *p_level );

void unpack_finish_lines16( QmfRegionLevel *p_level );

void unpack_free_lines16( QmfRegionLevel *p_level );
...
extern NCSError	build_qmf_compress_file(QmfLevel *p_top_qmf, NCS::CIOStream *pStream, NCS::ECW::CFile *pFile, NCS::CView_CWriterBase *pWriter);
//for 16bit bands except opacity band
extern NCSError	build_qmf_compress_file16(QmfLevel *p_top_qmf, NCS::CIOStream *pStream, NCS::ECW::CFile *pFile, NCS::CView_CWriterBase *pWriter);

extern NCSError	build_qmf_level_qencode_line(QmfLevel *p_qmf, UINT32 y, IEEE4 **p_p_ll_line);
//for 16bit bands except opacity band
extern NCSError	build_qmf_level_qencode_line16(QmfLevel *p_qmf, UINT32 y, IEEE4 **p_p_ll_line);

extern NCSError qencode_qmf(QmfLevel *p_top_qmf, int num_levels,
			  FLOAT compression_factor, NCS::CIOStream *pStream);

extern NCSError	pack_data(UINT8 **p_packed,	UINT32 *packed_length,
					  UINT8 *p_raw,		UINT32 raw_length, UINT8 symbol_size_hint, BOOLEAN bTryHuffman);
// 16bit version
// nECWhuffmancodingFlag = 0, no huffman coding
// nECWhuffmancodingFlag = 1, count ZEROS, pack non-zeros and zeros 32 to 8 bit data, most compression ratio, second fastest decoding, but second slowest encoding
// nECWhuffmancodingFlag = 2, count ZEROS, pack non-zeros and zeros 32 to 16 bit data, third compression ratio, but slowest encoding and decoding
// nECWhuffmancodingFlag = 3, first shift to 32 bit unsigned, pack 8 to 8 bit data. fastest encoding and decoding. but may be less compression ratio (when there are many same value neighboured)
// nECWhuffmancodingFlag = 4, count ZEROS, pack non-zeros and zeros 8 to 8 bit data. fastest encoding and decoding. compression ratio is ok
// nECWhuffmancodingFlag = 5, count ZEROS, pack non-zeros and zeros 16 to 8 bit data. third slowest encoding and fastest decoding. compression ratio is ok
// nECWhuffmancodingFlag = 6, count NONDIFFS, pack NONDIFFS and DIFFS 8 to 8 bit data. fastest encoding and decoding. compression ratio is ok
// nECWhuffmancodingFlag = 7, count NONDIFFS, pack NONDIFFS and DIFFS 16 bit data to 8 bit data. fastest encoding and decoding. seond most compression ratio
extern NCSError	pack_data16(UINT8 **p_packed,	UINT32 *packed_length,
					  UINT8 *p_raw,		UINT32 raw_length, UINT8 symbol_size_hint, UINT8 nECWhuffmancodingFlag = 1);
...
NCSError write_compressed_preamble(QmfLevel *p_top_qmf, NCS::CIOStream *pStream,NCS::ECW::CFile *pFile);
// convert the block table to LSB offsets
// note file_offset and pStream can be "main" or "opacity"
NCSError convert_block_table(QmfLevel *p_top_qmf, UINT64 file_offset, NCS::CIOStream *pStream);
// writes the level to the compressed ERW file
NCSError write_compressed_level(QmfLevel *p_qmf, NCS::CIOStream *pStream);

// writes the preamble header in the compressed ERW file
NCSError write_compressed_preamble_opacity(QmfLevel *p_top_qmf, NCS::CIOStream *pStream);
// writes the level to the compressed ERW file
NCSError write_compressed_level_opacity(QmfLevel *p_qmf, NCS::CIOStream *pStream);
// adds the opacity file onto the end of the output ecw file
NCSError add_opacity_to_output_file(QmfLevel *p_qmf, NCS::CIOStream *pStream, NCS::CIOStream *pOpacityStream);
// need to unparse all variable boxes after encoding
NCSError unparse_boxes_after_encoding(NCS::CIOStream *pStream, UINT64 oldOffset, NCS::ECW::CFile *pFile);
...
extern QmfLevel *erw_decompress_open(
			NCS::CIOStream *pStream,		// File to open
...
extern QmfLevel *erw_decompress_open_opacity(
			NCS::CIOStream *pStream,		// File to open
			BOOLEAN bReadOffsets,			// TRUE if the client wants the block Offset Table
			BOOLEAN bReadMemImage,			// TRUE if the client wants a Memory Image of the Header
			UINT64 first_stream_length);	// length of the first stream (before this one)

extern QmfRegion *erw_decompress_start_region( QmfLevel *p_top_qmf,
				 UINT32 start_x, UINT32 start_y,
				 UINT32 end_x, UINT32 end_y,
				 UINT32 number_x, UINT32 number_y,
				 UINT8 nCellBitDepth = 8);
...
void erw_decompress_end_region( QmfRegion *p_region );
extern void erw_decompress_close( QmfLevel *p_top_qmf );
...
extern int qdecode_qmf_level_line( QmfRegion *p_region, UINT16 level, UINT32 y_line,
								  NCSECW_BUFFER_TYPE **p_p_output_line);
// for 16bit version
extern int qdecode_qmf_level_line16( QmfRegion *p_region, UINT16 level, UINT32 y_line,
								  NCSECW_BUFFER_TYPE_32 **p_p_output_line);
